...
_SEM_CAMINHO_HOJE: dict[str, str] = {
    'integrations/jogos_locais.py::jogo_da_janela':
        'A VOLTA DO CAMINHO QUE SÓ A STEAM TINHA: de uma `wm_class` '
        '(`gotg.exe`), QUE JOGO É — o espelho de '
        '`profiles/steam_app.steam_appid_from_wm_class`, que o «Detectar» '
        'já usa na ida. A TELA DA ABA 10 SAIU DA POSSE DA '
        'JOGOS-DOS-LANCADORES-01 EM 11/09/2026 e está com a '
        'PERFIS-A-TELA-01, que corre ANTES: a sprint passou a entregar o '
        'MOTOR, e o `nao_toca` do frontmatter dela nomeia '
        '`interface/pacotes/a10_perfis.py`. ONDE O CAMINHO SE PERDE, e são '
        'DUAS linhas com endereço: `a10_perfis.py:2225`, dentro de '
        '`_jogo_reconhecido` (`frase_do_campo_do_jogo(texto, '
        '_nomes_dos_jogos())` precisa do terceiro argumento '
        '`nomes_das_janelas()`), e `a10_perfis.py:2913`, o último `return` '
        'de `detectar` (`_agora_vale_em(prof)` precisa de '
        '`_agora_vale_em(prof, classe)`). O que FECHA: a costura da onda '
        '0911 aplicando essas duas linhas depois da PERFIS-A-TELA-01 — as '
        'duas estão escritas com o diff em '
        '`docs/process/agentes/2026-09-11/JOGOS-DOS-LANCADORES-01-opus.md`, '
        'e o efeito delas foi medido em processo com as funções REAIS da '
        "aba: hoje o «Detectar» responde 'PRAGMATA' a um jogo da Steam e "
        'NADA a um jogo do Heroic, que é a queixa dela de 11/09. No dia em '
        'que a costura entrar, este portão cobra que estas entradas sejam '
        'APAGADAS. 11/09/2026.',
    'integrations/jogos_locais.py::nomes_das_janelas':
        'O `{wm_class: nome}` MEMOIZADO das três origens de fora da Steam, '
        'que é a metade que lê o disco (a decisão é `jogo_da_janela`, '
        'pura). É ele que entra como terceiro argumento na linha 2225. A '
        'TELA DA ABA 10 SAIU DA POSSE DA JOGOS-DOS-LANCADORES-01 EM '
        '11/09/2026 e está com a PERFIS-A-TELA-01, que corre ANTES: a '
        'sprint passou a entregar o MOTOR, e o `nao_toca` do frontmatter '
        'dela nomeia `interface/pacotes/a10_perfis.py`. ONDE O CAMINHO SE '
        'PERDE, e são DUAS linhas com endereço: `a10_perfis.py:2225`, '
        'dentro de `_jogo_reconhecido` (`frase_do_campo_do_jogo(texto, '
        '_nomes_dos_jogos())` precisa do terceiro argumento '
        '`nomes_das_janelas()`), e `a10_perfis.py:2913`, o último `return` '
        'de `detectar` (`_agora_vale_em(prof)` precisa de '
        '`_agora_vale_em(prof, classe)`). O que FECHA: a costura da onda '
        ...
        'que a costura entrar, este portão cobra que estas entradas sejam '
        'APAGADAS. 11/09/2026.',
    'integrations/jogos_locais.py::ofertas_do_campo_do_jogo':
        'A JUNÇÃO das duas origens do `<datalist>` do campo «Nome do Jogo» '
        '(Steam + lançadores). Ela TINHA chamador em '
        '`a10_perfis._html_dos_jogos` na passada de 10/09/2026, e o perdeu '
        'quando a tela saiu da posse. Volta com a mesma costura. A TELA DA '
        'ABA 10 SAIU DA POSSE DA JOGOS-DOS-LANCADORES-01 EM 11/09/2026 e '
        'está com a PERFIS-A-TELA-01, que corre ANTES: a sprint passou a '
        'entregar o MOTOR, e o `nao_toca` do frontmatter dela nomeia '
        '`interface/pacotes/a10_perfis.py`. ONDE O CAMINHO SE PERDE, e são '
        'DUAS linhas com endereço: `a10_perfis.py:2225`, dentro de '
        '`_jogo_reconhecido` (`frase_do_campo_do_jogo(texto, '
        '_nomes_dos_jogos())` precisa do terceiro argumento '
