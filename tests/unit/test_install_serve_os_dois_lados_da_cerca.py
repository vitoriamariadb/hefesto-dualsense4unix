...
#: preâmbulo, o lado dos formatos e o lado nativo — continuam saindo só do
#: `install.sh`, porque a cerca é dele. O que o portão mede é o mesmo de
#: sempre: se a chamada de cada cura chega aos dois lados daquela cerca.
...
def _corpos(linhas: list[str]) -> tuple[dict[str, list[str]], list[bool]]:
    ...
    Era o miolo de `corpos_e_topo`; virou função própria quando as curas de
    HOST mudaram de arquivo, para que o MESMO leitor sirva os dois — um leitor
    por arquivo seria duas chances de divergir, e a divergência aqui não
    aparece como erro: aparece como portão que aprova sem olhar.
    """
...
def corpos_das_duas_casas() -> dict[str, list[str]]:
    ...
    O fecho transitivo do alcance precisa dos dois: a chamada mora no
    `install.sh` e o corpo da cura mora na lib. Ler um só faria o fecho parar
    na primeira `*_host` — e um fecho que para cedo só sabe ACUSAR FALSO, que
    é a direção de erro que este arquivo inteiro existe para evitar.
    """
