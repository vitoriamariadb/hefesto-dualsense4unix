...
**A tela promete um reparo manual e não oferece nenhum caminho para fazê-lo** — não existe UM botão de copiar em toda a interface nova. E uma frase do produto manda ela a um botão que não existe.

**A fonte das decisões abaixo é**
[O PO DECIDE AS 54](../2026-09-04-O-PO-DECIDE-as-54-e-os-sete-conflitos.md) — §2, aba `07-lancadores`. Onde há a marca de um **conflito C-n**, a recomendação escrita na lista da aba propunha o CONTRÁRIO de uma decisão que ela já tinha tomado; **siga o que está aqui**, e a §1 daquele documento diz por quê.
...
### [01] o jogo que só dá para consertar à mão

**Os dois, só quando faz falta** — o botão «Copiar a linha» E a linha à mostra, só no estado em que o cartão já diz «linha intocável». Se a cópia falhar em silêncio, a linha à mostra ainda salva; no dia bom, nada disso ocupa a tela.

### [02] a frase que manda a um botão inexistente

**A frase para de nomear lugar.** Vira só o fato — *«O jogo está rodando sem o hefesto-launch — controles podem duplicar.»* — e quem diz o que fazer é o botão ao lado, em cada tela. É a única que fica certa nas DUAS telas, e cumpre a regra dela: uma frase, um dono.

### [03] de onde o aviso some

**As duas recusas calam tudo.** «Não perguntar para este jogo» e «Tirar daqui» silenciam o aviso no cartão E na coluna Atenção da Jogar. Um aviso que sobrevive à resposta dela ensina que o botão não obedece. **A leitura das duas listas vem de vigia em segundo plano** — esta aba já tem uma —, nunca no caminho do tique.

### [04] os dois números do cartão da Steam

**O corpo nomeia o conjunto** — «…da sua biblioteca (instalados ou não)». Três palavras, e resolve o que falta: dizer que os dois números contam coisas diferentes.
...
* 'Deixar tudo pronto' — Steam Input + wrapper com UM consentimento
* 'Este jogo não funciona' — a allowlist do Steam Input
* Repor o atalho de inicialização DE CARONA ao Salvar/Aplicar perfil
...
`docs/process/agentes/2026-09-04/ONDA2-07.md` <!-- ref-externa: esta sprint CRIA o relatório; ele nasce no fim da frente --> , com quatro cabeçalhos:
