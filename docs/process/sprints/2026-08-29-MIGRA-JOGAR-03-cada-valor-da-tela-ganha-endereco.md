...
bancada: false
depois_de:
  # SÉRIE, por R5: as duas abrem `novo-layout/_ferramentas/aba01.py`, que a 02
...
não tem nenhum. Uma varredura por `id=` em `novo-layout/01-jogar.html` devolve
**só** `<linearGradient>`, `<filter>`, `<defs>`, `<pattern>` e `<style>` — todos
de dentro do SVG, todos gerados pelo `novo-layout/_ferramentas/monta.py`. **Nem
