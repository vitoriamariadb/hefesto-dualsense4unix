...
- **A palavra dela sobre esta aba:**
  `src/hefesto_dualsense4unix/interface/CORRECOES-DELA.md`, seção "Aba Sistema".
...
* **02 é solta e pode correr do primeiro minuto** — ela só toca
  `src/hefesto_dualsense4unix/interface/aba09.py`, que nenhuma outra sprint desta casa abre.
...
10. **`novo-layout/` é `.gitignore:108`.** Não viaja em `git worktree add`, não
    ...
    que a régua da 02 vive em `src/hefesto_dualsense4unix/interface/`.
