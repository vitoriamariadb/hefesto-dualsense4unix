...
> **ESTADO 2026-09-06: feita** — a P1 fechou: o piso da procedência é ZERO (as 30 células que declaravam `de_onde_sei` sem apontar prova ganharam endereço, `grep` ou ensaio, e as SETE que diziam `idem` no `radio_codigo_ref` — um ponteiro que ocupa a coluna sem apontar — ganharam o endereço que a palavra escondia); nasceu a régua `tests/unit/test_a_procedencia_da_linha_nao_e_vazia.py` com cinco nós e cinco mordidas provadas; a pergunta *de onde se sabe esta célula* ganhou DONO em `check_paridade_transporte.procedencia_da_celula()`, com o nó 5 guardando a divergência com a `mesa_de_medicao`; nasceu a regra 20 (`causa-sem-negativa`, AVISO) e ela achou QUATRO células onde a A-RECUSA §4.5 previa uma; as cinco pequenas da A-RECUSA §4 fecharam (inclusive os TRÊS endereços mortos da emulação de mouse); e as nove células que a leva de hoje exercitou ganharam `MONTOU` com o relatório apontado (as duas da `combinacao.slot_jogador.estabilidade` ficaram sem o degrau porque uma régua de 03/09 o proíbe). A P2 (medir no aparelho) continua dela, na MESA-DE-QUATRO-01. Entrega em `docs/process/agentes/2026-09-06/SPECS-A-PROCEDENCIA-01-opus.md`.
