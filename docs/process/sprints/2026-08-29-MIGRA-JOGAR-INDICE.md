...
| 04 | **A cor do plástico nos cartões do rádio.** O mapa é portão e diz **não** (`docs/data/mapa-controles.csv:111`, `radio_aciona=não`, motivo `divida` desde 29/08/2026, quando a lápide `o-aparelho-recusa` caiu; `cabo_aciona=sim`, e a leitura pelo cabo só passou a acontecer de verdade em 29/08, com a porta do broker). Três saídas, de preços diferentes: **lembrar** o que se leu no cabo (`ControleDeclarado.cor`, `utils/maquina.py:551`), **nascer sem cor**, ou **ela declarar à mão**, que é o que o produto já permite. **NOTA DE 06/09/2026 (A-RECUSA-QUE-CITOU-O-MAPA-01): o mapa deixou de dizer não.** A `ONDA-CONEXOES-11` correu em 02/09/2026 (commit `2e772412`) e a mesma linha 111 é hoje `radio_aciona=sim`, `radio_de_onde_sei=medido`, `radio_ate_onde_foi=SAIU NO FIO` — o produto lê a cor pelo rádio. As três saídas viram recurso para a unidade que não responder, não caminho obrigatório do transporte; ressalva viva do mapa é de **amostra** (duas unidades, não quatro) |
