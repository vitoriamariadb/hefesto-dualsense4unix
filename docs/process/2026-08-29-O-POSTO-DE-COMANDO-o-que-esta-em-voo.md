...
```bash
src/hefesto_dualsense4unix/interface/controles_vivos.py     # a aba Controles, com a mesa dela
```
...
3. **Os botões da janela seguem o estilo do COSMIC** — resolvido com
   `Gtk.HeaderBar` no `ver.py`; sem ela a decoração vinha do compositor e os  <!-- ref-externa: mora em `novo-layout/`, que é .gitignore e NÃO viaja em worktree -->
...
```
src/hefesto_dualsense4unix/interface/ver.py        # abre na Jogar; a tira do desenho navega
src/hefesto_dualsense4unix/interface/ver.py 02     # abre já na Controles
```
...
- **Janela só no workspace `OS`**, por `aurora-claude-workspace.sh run`, ou
  `Gtk.OffscreenWindow`. Exceção: o `ver.py`, que **ela** manda abrir.  <!-- ref-externa: mora em `novo-layout/`, que é .gitignore e NÃO viaja em worktree -->
