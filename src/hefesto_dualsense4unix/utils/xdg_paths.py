...
# O SLUG, e ele carrega SETE recursos de uma vez: config, data, cache, runtime,
# state, perfis e o diretório do socket IPC saem todos daqui. Por isso ele não
# se digita neste arquivo — vem de `utils/identidade.py`, que é o dono.
_DIRS = PlatformDirs(identidade.atual().slug)

# O NOME do socket é LITERAL de propósito: `tests/unit/test_doc_verdade_02_
# contagens_derivadas.py` lê estas duas constantes como texto para conferir se a
# documentação do protocolo cita o socket certo, e uma f-string aqui cega essa
# régua (medido em 29/08, ela reprovou).
