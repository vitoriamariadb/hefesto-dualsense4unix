...
def _teto_do_controle(
    ...
    (`profiles/schema.py:1570-1611`), logo é o que está no disco; procurar por
...
def mic_existe(ctx: Contexto, o: dict[str, Any], p: Any) -> None:
    ...
    *"Desligado, nenhum programa o enxerga"* — e isso é a PONTE, que existe ou
    ...
    (`ipc_handlers.py:6678`, QUATRO-MICROFONES-01): *"o 'Aplicar' tem de VALER
...
def _com_o_teto(prof: Any, chave: str, policy: str | None) -> Any:
    ...
    `cfg.rumble is None` (`profiles/manager.py:2106`) e `"policy" not in
    ...
    1,0 (`profiles/manager.py:2295-2298`) — guardar o override só deixaria no
...
# `_handle_machine_declare` (`daemon/ipc_handlers.py:6678`) → `maquina.json`, e
