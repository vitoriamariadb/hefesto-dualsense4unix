...
CSS = """
  ...
  /* A CONTA DA MARGEM, e ela é apertada por CONSTRUÇÃO. `4px 4px 0` em vez do
     ...
     MEDIDO no WebKit da janela do produto (1180x757), com a MESA DELA — dois
     controles, um `usb` e um `bt` —, injetando o BOOTSTRAP do piloto e pintando
     a carga do `a05_vibracao.pacote()`:

       .miolo:  client 564 · padding 16 em cima e 18 embaixo -> 530 de conteúdo
       ANTES da pintura (a cena CRAVADA, 1 linha):  quadro 528 · rola 0
       DEPOIS da pintura (a mesa dela, 2 linhas):   quadro 570 · rola 40

     São DOIS px de folga no desenho, e o estado dela custa 42. A segunda linha
     do alerta — "que você fixar aqui embaixo." — fica CORTADA pela borda de
     baixo do miolo, com a barra de rolagem à direita. Fotografado.

     E `vpads == 0` NÃO É EXCEPCIONAL: é o estado corrente da máquina dela, com
     os dois controles na mesa. O aviso que esta linha existe para dar é
     exatamente o que fica pela metade.

     O QUE FALTA É DECISÃO DELA, e não pixel: caber os 42 px exige encolher a
     tabela que ela aprovou, e o `.miolo` (16px 3px 18px 18px) é do `topo.html`,
     comum às dez abas. As três saídas — encolher uma linha da tabela, pôr o
     bloco no TOPO do quadro (aí quem sai de vista é o "Parar"), ou aceitar a
     rolagem — estão em `mockup/DIVERGENCIAS.md`, esperando o OK. */
