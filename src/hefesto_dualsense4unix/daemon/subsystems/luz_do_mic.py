...
série (`core/backend_pydualsense.py:4373`, `flag = bool(aceso)`, e `:372`,
...
def _mudo(backend: Any, uniq: str) -> bool | None:
    ...
    de INPUT (`core/backend_pydualsense.py:4450`). **Não é `microphone_mute_for`
...
def _baterias(backend: Any) -> dict[str, int]:
    ...
    (`core/backend_pydualsense.py:5918`) e a leitura é `getattr` no objeto que
...
def _escrever(backend: Any, uniq: str, valor: int | None) -> bool:
    ...
    **NÃO É `set_mic_led`.** Aquele esmaga em `bool` duas vezes em série e faz
    ...
    `core/backend_pydualsense.py:4373` e `:372`).
