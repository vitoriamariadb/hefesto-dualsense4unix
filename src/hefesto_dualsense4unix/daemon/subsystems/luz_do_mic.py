...
série (`core/backend_pydualsense.py:4039`, `flag = bool(aceso)`, e `:372`,
...
def _mudo(backend: Any, uniq: str) -> bool | None:
    ...
    de INPUT (`core/backend_pydualsense.py:4077`). **Não é `microphone_mute_for`
...
def _baterias(backend: Any) -> dict[str, int]:
    ...
    (`core/backend_pydualsense.py:5504`) e a leitura é `getattr` no objeto que
...
async def _fora_do_laco(daemon: Any, fn: Any, *args: Any) -> Any:
    ...
    executor montado (`daemon/lifecycle.py:4874` afirma isso), e um daemon
...
def _escrever(backend: Any, uniq: str, valor: int | None) -> bool:
    ...
    **NÃO É `set_mic_led`.** Aquele esmaga em `bool` duas vezes em série e faz
    ...
    `core/backend_pydualsense.py:4039` e `:372`).
